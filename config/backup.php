...
return [
    ...
    'backup' => [
        ...
        'source' => [
            'files' => [
                /*
                 * Sección 9 y "Principio rector #2" del SPEC: lo crítico
                 * es la base de datos (ver 'databases' abajo), no el
                 * código — el código ya vive en git. Respaldar
                 * base_path() completo sería redundante y voluminoso.
                 * Lo único que se respalda aparte de la base de datos
                 * es storage/app: ahí viven archivos generados que NO
                 * están en git (logos de clientes subidos por
                 * FileUpload, PDFs de reportes ya generados).
                 */
                'include' => [
                    storage_path('app'),
                ],
        ...
        'destination' => [
            ...
            /*
             * El disco 'local' solo sirve para desarrollo: sección 9
             * del SPEC exige almacenamiento EXTERNO ("respaldos
             * automáticos diarios... a almacenamiento externo") — en
             * producción, BACKUP_DISK debe apuntar a un disco que no
             * viva en el mismo servidor (ej. 's3', ya configurado en
         ...
         *
         * env()'s propio default no sirve aquí: BACKUP_ARCHIVE_PASSWORD=""
         * (vacío pero presente en .env) no cuenta como "ausente" para
         * env(), así que sin el operador Elvis quedaría como cadena
         * vacía en vez de null — y una cadena vacía SÍ activa el
         * cifrado (con contraseña vacía), rompiendo ZipArchive::close().
         */
    ...
    'notifications' => [
        ...
        'mail' => [
            // env()'s propio parámetro por defecto no sirve aquí:
            // BACKUP_NOTIFICATION_EMAIL="" (vacío pero presente) no
            // cuenta como "ausente" para env(), así que el default no
            // se activaría y este paquete valida el email al arrancar
            // cualquier comando artisan, no solo al enviar un correo.
